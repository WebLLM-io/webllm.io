...
**Parameters:**
- `modelId` (string) - MLC model identifier (e.g., `'Qwen3-8B-q4f16_1-MLC'`)
...
Returns the current runtime status of the client, including which model is resolved, whether local inference is ready, and which engines are enabled.
...
- `localModel` — Resolved local model ID, or `null` if no model is selected
- `localReady` — `true` if the local model is loaded and ready for inference
- `localEnabled` — `true` if local inference is configured
- `cloudEnabled` — `true` if cloud inference is configured

**Example:**

```ts
const status = client.status();
console.log(`Local model: ${status.localModel}`);
console.log(`Local ready: ${status.localReady}`);
console.log(`Local enabled: ${status.localEnabled}`);
console.log(`Cloud enabled: ${status.cloudEnabled}`);
```
