...
const client = createClient({
  local: {
    tiers: {
      high: 'Qwen3-8B-q4f16_1-MLC',         // S/A grade (≥4GB VRAM)
      medium: 'Qwen2.5-3B-Instruct-q4f16_1-MLC',        // B grade (≥2GB VRAM)
...
const isCached = await client.hasModelInCache('Qwen3-8B-q4f16_1-MLC');

// Delete model from cache
await client.deleteModelFromCache('Qwen3-8B-q4f16_1-MLC');
```
...
The `stats` object contains `{ grade, gpu, battery, memory }`.
