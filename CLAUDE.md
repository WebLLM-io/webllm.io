...
- **Sidebar two-zone layout (chat)** — Sidebar is split into two vertical zones: (1) conversation list with independent `overflow-y-auto` scrolling (`flex-1 min-h-0`), and (2) a pinned bottom zone (`shrink-0 max-h-[50%]`) containing status card, collapsible settings, and footer. The bottom zone's `max-h-[50%]` cap ensures the conversation list always has at least ~50% of sidebar height. When settings expand beyond available space, the bottom zone scrolls internally. Footer is pinned at the very bottom of the bottom zone with `shrink-0`.
